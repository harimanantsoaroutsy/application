...
$("html").on("click","#btnajout",function(e){
   e.preventDefault();
 $('#myModal').modal('show');
  $('#myModal .modal-header').html('<h4 style="text-align:center;color:white">NOUVELLE MATIERE</h4>');
...
$("html").on('click',"#ajout",function(e){
 ...
 $('#myForm').bootstrapValidator("revalidateField","nom");
	$('#myForm').bootstrapValidator("revalidateField","coef");
   ...
   success: function() {   
      $('#myModal').modal('hide');
	$('#content').load('matiere.php #mondiv');
...
$("html").on("click","#btnupdate",function(e){
   e.preventDefault();
 $('#myModal').modal('show');
  $('#myModal .modal-header').html('<h4 style="text-align:center;color:white">MODIFICATION MATIERE</h4><input type="hidden" id="num" />');
  $('#myModal .modal-footer').html('<button class="btn btn-primary" style="height:35px" id="update">Modifier</button><button class="btn btn-primary push" style="height:35px" id="annulation">Annuler</button>');
   ...
   numero= $("#tabMatiere").find('tr').eq(numLigne).find('td').eq(0).text();
   nom= $("#tabMatiere").find('tr').eq(numLigne).find('td').eq(1).text();
   coef= $("#tabMatiere").find('tr').eq(numLigne).find('td').eq(2).text();
 $("#nom").val(nom);
 $("#coef").val(coef);
  $("#num").val(numero);
...
$("html").on("click","#btnsuppr",function(e){
   e.preventDefault();
   $('#myModal1').modal('show');
  $('#myModal1 .modal-header').html('<h4 style="text-align:center;color:white">Voulez-vous vraiment supprimer cette maitère?</h4>');
